...
void place(unsigned int i, std::vector<particle> &particles, Vector center, double sidelength, cube* current) {
    ...
    if (id >= 0){ //If the current cube has an occupant
        ...
        current->rmv_from_cube(particles[id]);//reverse the CoM and mass because it will be r>
        current->set_id(-1);
        --level;
...
Matrix calculate_quadrupole(Vector com, unsigned int &n, cube* current){
    ...
    for (int i = 0; i < 8; ++i){
        ...
        if (next) {
            Q += calculate_quadrupole(com, n, current->next(i));
        }
    }
    return Q;
}

double calculate_radius(Vector com, unsigned int n, cube* current){
    double r = 0;
    if (current->id() >= 0){
    ...
    for (int i = 0; i < 8; ++i){
        cube* next = current->next(i);
        if (next) {
            r += calculate_radius(com, n, current->next(i));
        }
    }
    return r;
}
...
void calculate_properties(cube* current){
    unsigned int n = 0;
    //calculate the quadrupole and count how many particles are in the cube
    Matrix Q = calculate_quadrupole(current->com(), n, current);
    current->set_quadrupole(Q);
    //calculate the average distance from the com
    double r = calculate_radius(current->com(), n, current);
...
int main() {
    ...
    */
    ...
    /*
    for (unsigned int i = 0; i < its; ++i){
        //Might have to make sure it doesn't calculate forces with itself
        //it might divide by 0... simple check with i and the cube id should do
        calculate_forces(particles[i], i, head);
    }
    */
    ...
    for (int i = 0; i < 8; ++i){
        for (int j = 0; j < 8; ++j){
            cube* next = head->next(i)->next(j);
            if (next) std::cout << "Q" << i << "," << j << ": \n" << next->Q() << std::endl;
        }
    }
    std::cout << "Final r: " << head->radius() << std::endl;
    for (int i = 0; i < 8; ++i){
        for (int j = 0; j < 8; ++j){
            cube* next = head->next(i)->next(j);
            if (next) std::cout << "r" << i << "," << j <<": \n" << next->radius() << std::endl;
        }
    }
    */
