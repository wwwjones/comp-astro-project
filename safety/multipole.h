...
class cube {
    public:
        cube() : id_(-1), center_(Vector::Zero()), sidelength_(0.),
            com_(Vector::Zero()), radius_(0.), mass_(0.),
        ...
        cube(particle p, int id, Vector center, double sidelength) :
            id_(id), center_(center), sidelength_(sidelength),
            com_(p.position()), radius_(p.position().norm()), mass_(p.mass()),
            quadrupole_(Matrix::Zero()) {
            ++total_p;
            ++total_c;
        ...
        //getters
        int id() {return id_;}
        double sidelength() {return sidelength_;}
        ...
        ~cube() {
            id_ = sidelength_ = radius_ = mass_ = 0;
            center_ = com_ = Vector::Zero();
            quadrupole_ = Matrix::Zero();
    ...
    private:
        ...
        //Might not need center...
        Vector center_;
        //sidelength might not actually be needed...
        //After the first one it is implicitly defined but then never needed
        double sidelength_;
        Vector com_;
        double radius_;
        double mass_;
